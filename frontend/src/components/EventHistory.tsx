  ...
  const combineConsecutiveEvents = (events: Event[]): Array<Event & { duration?: number; count?: number }> => {
    ...
    for (let i = 1; i < events.length; i++) {
      ...
      if (prevEvent.is_drowsy === currentEvent.is_drowsy) {
        ...
        const duration = Math.floor((mostRecentTime - oldestTime) / 1000); // duration in seconds
          ...
          {combinedEvents.length === 0 ? (
            ...
            <div className="event-list">
              {combinedEvents.map((event, index) => {
                const avgScore = event.drowsiness_score; // Use first event's score as representative
