...
export const CameraFeed = forwardRef<CameraFeedRef, CameraFeedProps>(({
  ...
  const startCamera = useCallback(async () => {
    if (!hasActiveSession) {
      return; // Don't start camera without active session
    }
  ...
  const captureFrame = useCallback(async () => {
    ...
    try {
      ...
      const result = await detectionAPI.detect(base64, timestamp, sequenceNumber);
      setHasDetected(true);
  ...
  }, [isDetecting, isStreaming, onDetectionResult, onError]);
